...
// Route per creazione risposta a recensione evento
/**
 * @openapi
...
router.post('/createAnswerEventReview', async (req, res) => {
 ...
 let evento = await Event.findOne({_id: req.body.idEvento});
    ...
    // Crea la risposta alla recensione evento
  	let answerEventReview = new AnswerEventReview({
  	...
  	answerEventReview = await answerEventReview.save();
  	res.status(201).json({
    		success: true,
...
// Route per creazione risposta a recensione alloggio
/**
 * @openapi
