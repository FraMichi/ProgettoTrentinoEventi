...
// Route per filtrare gli alloggi
/**
 * @openapi
 * /api/v2/visualizzazioneFiltrata/getFilterHousings:
 *  get:
 *   description: Resitituisce gli alloggi che soddisfano i filtri applicati
 *   summary: Filtra gli alloggi
 *   tags:
 *    - housingFiltering
 *   responses:
 *    200:
 *     description: non ci sono errori e gli alloggi sono stati filtrati correttamente
 *     content:
 *      application/json:
 *       schema:
 *        properties:
 ...
 *         housingsList:
 *          type: string
 *          description: Contiene la lista degli alloggi filtrati
 *    400:
 *     description: Restituisce errore se il formato delle date non è corretto e la data di inizio è maggiore di quella di fine
 *     content:
 *      application/json:
 *       schema:
 *        properties:
 ...
 *          description: Messaggio che contiene l'errore
 */
router.get('/getFilterHousings', async (req, res) => {
    ...
    // Prendo dal DB tutti gli alloggi applicando i filtri
    let housings = await Housing.find(dict).exec();

    // Creo la lista di alloggi da ritornare, solo con id e titolo
    let housingsList = housings.map((housingItem) => {return{id: housingItem._id, title: housingItem.titolo};})

    res.status(200).json({success: true, housingsList: housingsList});
});
